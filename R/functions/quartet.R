...
quad <- function(p1, p2, p3, p4, plot.title, ncols = NULL, start.date = '2000-01-01'){
  p <- dplyr::left_join(read_data(p1), 
                        read_data(p2), 
                        by = 'date') %>% 
    dplyr::left_join(read_data(p3), by = 'date') %>% 
    dplyr::left_join(read_data(p4), by = 'date') %>% 
